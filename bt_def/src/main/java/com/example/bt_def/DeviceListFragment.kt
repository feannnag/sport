...
class DeviceListFragment : Fragment(), ItemAdapter.Listener{
    ...
    private val requestMultiplePermissions =
        registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
            permissions.entries.forEach {
                Log.d("Debugging", "${it.key} = ${it.value}")
                btLauncher.launch(Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE))
            }
        }
    ...
    private fun getPairedDevices(){
        try {
            ...
            deviceList.forEach{
                ...
                /*if (preferences?.getString(BluetoothConstants.MAC, "") == it.address){
                    Log.d("Debugging", "Name: ${it.name} MAC = ${it.address} ")
                    btConnection.connect(it.address)
                }*/
            }
    ...
    private fun bluetoothState(){
        if (bAdapter?.isEnabled == true){
            changeButtonColor(binding.imBluetoothOn, Color.WHITE)
            getPairedDevices()
        }
    }
    ...
    private fun registerPermissionListener(){
        ...
        pLauncher = registerForActivityResult(
            ...
            Snackbar.make(binding.root, "Необходимо разрешение для совершения поиска!", Snackbar.LENGTH_LONG).show()
        }
    }
    ...
    private val bReceiver = object : BroadcastReceiver(){
        override fun onReceive(p0: Context?, intent: Intent?) {
            if (intent?.action == BluetoothDevice.ACTION_FOUND){
                ...
                try {
                    Log.d("Debugging", "Device: ${device?.name}")
                } catch (e: SecurityException){
